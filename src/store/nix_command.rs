...
impl StoreBackend for CommandBackend {
  ...
  fn query_closure_size(&self, path: &Path) -> Result<Size> {
    let system_path = path.join("sw");
    let system_path = system_path.to_string_lossy();
    let cmd_res = command_output(&self.nix_cmd, &[
      "path-info",
      "--closure-size",
      &system_path,
    ])
...
/nix/store/0m8p1yj6k5fk7fpvj37krhbsnry8v70r-pmx-3.00-tex";
  ...
  fn setup_fake_nix_command(content: &str) -> (TempDir, String) {
    ...
    {
      let mut file = std::fs::File::create(&mock_command)
        .expect("unable to create mock command file");
      file
        .write_all(content.as_bytes())
        .expect("unable to write mock command file");
      file.sync_all().expect("unable to sync mock command file");
    }
